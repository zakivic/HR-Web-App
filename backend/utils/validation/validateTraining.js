...
export const validateTraining = (training) => {
  ...
  if (training.name && validator.isEmpty(training.name)) {
    errors.push({ field: "name", message: "Name is required" });
  }

  // Check if instructor is provided
  if (training.instructor && validator.isEmpty(training.instructor)) {
  ...
  if (training.startDate && training.endDate) {
    ...
    if (isNaN(validStartDate.getTime()) || isNaN(validEndDate.getTime())) {
      errors.push({
        ...
        message: "Start date and end date must be in a valid format",
      });
    }
  ...
  if (training.location && validator.isEmpty(training.location)) {
  ...
  if (
    training.employees &&
    (!Array.isArray(training.employees) ||
      training.employees.some(
        (employee) => !mongoose.Types.ObjectId.isValid(employee)
      ))
  ) {
    errors.push({
      field: "employees",
      message: "Employees must be an array of valid ObjectIds",
    });
  }
